...
def run(doc_path, output_base_dir, stopwords_path, vocab_min_count, num_epochs, algorithm, vector_size, alpha, min_alpha, train, window, cores):
...
def main():
    ...
    parser.add_argument('--dim', help='word and document vector size', type=int, default=100)
    ...
    parser.add_argument('--cores', help='number of cores to train on', type=int)
    ...
    run(doc_path=args.input_path, output_base_dir=args.output_base_dir, stopwords_path=args.stopwords, vocab_min_count=args.vocab_min_count, 
        num_epochs=args.epochs, algorithm=args.algorithm, vector_size=args.dim, alpha=args.alpha, min_alpha=args.min_alpha, 
        train=args.train, window=args.window, cores=args.cores)
